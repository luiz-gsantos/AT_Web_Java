...
public class UsuarioLoader implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        userAdmin.setAdministrador(true);
        userAdmin.setCnh(1115552999);
        usuarioService.incluir(userAdmin);
        System.out.println("Inclusão do usuario "+userAdmin.getNome()+" realizada com sucesso!");

        for (int i = 0; i < 5; i++) {
            Usuario usuario = new Usuario("usuario " + i, "usuario"+i+"@user.com", "0"+i);
